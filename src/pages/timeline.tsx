...
export const getStaticProps: GetStaticProps<TimelineProps> = async () => {
    const { default: rawTimeline }: any = await import('../../data/timeline.json')
    const timeline = (rawTimeline as Array<TimelineEvent>).sort((a, b) => +new Date(b.date) - +new Date(a.date))
                                ...
                                <div className="relative pb-8">
                                    ...
                                    <div className="relative flex items-center space-x-3 bg-gray-50 bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-75 backdrop-filter backdrop-blur-sm px-2 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-lg">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <h1 className="flex flex-wrap justify-between mb-2 text-gray-900 dark:text-white text-lg tracking-tight font-bold">
                                                <span>{event.title}</span>
                                                <span className="flex-1 sm:hidden" />
                                                <PillDate className="mt-2 sm:mt-0" small={true}>
                                                    {format(event.date, 'PPP')}
                                                </PillDate>
                                            </h1>

                                            <p className="my-2 dark:text-gray-100 text-base">{event.description}</p>
