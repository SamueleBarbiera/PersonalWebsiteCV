...
function coerce<AOut, ADef, AIn, TOut, BOut, BDef>(
    ...
    return a.transform((value: AOut) => b.parse(transform(value)))
}

function refine<AOut, ADef, AIn, BOut, BDef>(a: z.Schema<AOut, ADef, AIn>, b: z.Schema<BOut, BDef, AOut>) {
...
const commaSeparated = coerce(z.string(), (value: any) => value!.split(','), z.array(z.any()))

const commaSeparatedNums = refine(commaSeparated, z.array(numeric()))

const json = coerce(z.string(), (value: any) => JSON.parse(value) as unknown, z.unknown())
...
const stringOrNum = z.transformer(z.string().or(z.number()), {
    type: 'transform',
    transform: (value: any) => (typeof value === 'string' ? parseInt(value, 10) : value),
})
