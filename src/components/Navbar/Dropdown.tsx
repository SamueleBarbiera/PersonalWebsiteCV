...
function MenuButtonIcon({ className, icon, direction: type = 'left' }: MenuButtonIconProps) {
    ...
    if (type === 'right') return <Icon aria-hidden="true" className={clsx('w-4 h-4 ml-3', className)} icon={icon} />
...
export default function Dropdown({ children, items, position = 'top-left' }: StandardProps) {
    return (
