...
const Track: FC<{ track: fetchers.SpotifyTrack }> = ({ track }) => {
    return (
        <Link href={track.url} passHref>
            <a href={track.url} className="p-5 stats-styles" rel="noreferrer" target="_blank">
        ...
        <Link href={artist.url} passHref>
            <a className="flex stats-styles" rel="noreferrer" target="_blank">
                ...
                <div className="flex flex-col items-start justify-center ml-5">
                    ...
                    <p className="text-xl">{artist.followers.toLocaleString()} followers</p>
                </div>
            </a>
        </Link>
    )
...
const Card: FC<CardProps> = ({ data, title, description, tracks }) => {
    return (
        <motion.div className="flex flex-col mt-24" variants={anims.FadeContainer} initial="hidden" animate="visible">
            ...
            <motion.div className="flex flex-col mt-10 overflow-hidden rounded" variants={anims.Fade}>
                {tracks
                    ? // @ts-ignore
                      data?.tracks?.map((track, index) => <Track key={index} track={track} />) // @ts-ignore
                    : data?.artists?.map((artist, index) => <Artist key={index} artist={artist} />)}
            </motion.div>
        </motion.div>
    )
}
